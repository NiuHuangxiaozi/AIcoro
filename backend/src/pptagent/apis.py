    ...
    def execute_actions(
        ...
        for line_idx, line in enumerate(api_calls):
            try:
                ...
                # func 告诉 eval：当你在 line 里看到这个函数名时，请用我给你的 partial_func 来代替它！
...
'''
test/test_apis.py .2025-11-23 22:10:13,251 - INFO - apis.py:270 -  
 =======================element.name: <p>这是一个<strong>加粗和<em>斜体</em>文本</strong>，还有<em>斜体和<code>Code def a+b</code></em>，<del>删除</del>，<a href="http://example.com">链接</a></p>
2025-11-23 22:10:13,252 - INFO - apis.py:270 -  
 =======================element.name: <p>这是一个<strong>加粗和<em>斜体</em>文本</strong>，还有<em>斜体和<code>Code def a+b</code></em>，<del>删除</del>，<a href="http://example.com">链接</a></p>
2025-11-23 22:10:13,252 - INFO - apis.py:270 -  
 =======================element.name: 这是一个
2025-11-23 22:10:13,252 - INFO - apis.py:270 -  
 =======================element.name: <strong>加粗和<em>斜体</em>文本</strong>
2025-11-23 22:10:13,252 - INFO - apis.py:270 -  
 =======================element.name: 加粗和

process_element的作用是将一个富文本转化为html，然后再用专业的工具读取转化为pptx能够接受的_Run对象
'''
...
def process_element(element, styles=None) -> list[TextBlock]:
    ...
    result = []
    # 如果全部是普通的文字，那么就直接创建TextBlock，这个Block会直接的构建pptx修改文字的最小单元，一个_Run
    if isinstance(element, str):
        ...
        if element.name == "a":
            ...
            # 必须使用 .copy()（或深拷贝，视情况而定），是为了避免不同递归分支之间意外共享和污染同一个 styles 字典。
            #  这里处理的情况是嵌套循环<strong>加粗和<em>斜体</em>文本</strong>
            new_styles = styles.copy()
            
            
            # 一个 <a> 标签下可能包含多个文本片段（即多个 TextBlock），每个都需要设置相同的 href
            for child in element.children:
                # 这里返回更加深层次的blocks回来，然后我们
                blocks = process_element(child, new_styles)
                for block in blocks:
                    block.href = element.get("href")
                result.extend(blocks)
                
        # 如果元素的名字符合加粗、斜体或者删除等等
...
def replace_para(paragraph_id: int, new_text: str, shape: BaseShape):
    ...
    # 用户友好输入（Markdown） → 机器友好结构（HTML） → 进一步处理（如转 PPT、PDF、网页等）
    html = markdown(new_text).strip()
    
    # 将一个 HTML 字符串解析成一个结构化的、可编程操作的文档对象模型（DOM 树），以便后续通过代码方便地提取、遍历或修改其中的元素。
    soup = BeautifulSoup(html, "html.parser")
    
    # 我们创建了多个TextBlock，每一个block对应于一个_Run
    ...
    '''
        假设 blocks 有 3 个（比如：[普通, 加粗, 链接]）。
        那么我们需要段落里有 3 个 Run。
        但现在只有 1 个空 Run（empty_run）。
        所以需要再创建 2 个新的空 Run。
        在addnext操作的时候我们就修改了para.runs
    '''
