...
class LLM:
    ...
    def format_message(
        ...
        # 如果是qwen3模型，我们还需要再每一次问问题的时候加上系统提示词，其他模型再都是在对话的最开始的时候加上就可以了
...
class AsyncLLM(LLM):
    ...
    def __post_init__(self):
        ...
        """
        logger.info(f"Initializing AsyncLLM with model: {self.model}, \n \
                    base_url: {self.base_url}, \n \
                    api_key: {self.api_key}, \n \
                    timeout: {self.timeout}")
    ...
    async def __call__(
        ...
        **client_kwargs,
        ...
        # import json
        # print(json.dumps(system, indent=4, ensure_ascii=False))
